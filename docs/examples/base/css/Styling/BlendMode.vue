      ...
      <el-form ref="formRef" label-width="auto">
        <el-form-item label="混合模式">
          <el-select size="small" v-model="selectedMode" @wheel="wheelToggle">
            <el-option v-for="mode in blendModes" :key="mode.name" :label="mode.name" :value="mode.name"/>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="渐变">
          <el-radio-group size="small" v-model="selectedGradient">
            <el-radio-button
                ...
                :value="gradient.name"></el-radio-button>
          </el-radio-group>
        </el-form-item>
      </el-form>
    </div>
...
const currentMode = computed(() => blendModes.value.find(n => n.name === selectedMode.value))
...
.blend-mode-demo {
  ...
  .blend-container {
    ...
    &.gradient-radial-purple {
      background-image: radial-gradient(circle, rgba(147, 51, 234, 0.5), rgba(79, 70, 229, 0.5)),
      $demo-bg-image;
    }

    // 绿色斜向渐变
    &.gradient-green-diagonal {
      background-image: linear-gradient(135deg, rgba(34, 197, 94, 0.5), rgba(6, 95, 70, 0.5)),
      $demo-bg-image;
    }
