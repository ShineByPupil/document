...
| `Web Push API`                            |    ↓     |    ~     |    ⚠️    | 通过浏览器推送服务发送通知（需 `ServiceWorker` 和用户授权）             |
| [`WebTransport`](WebTransport.md)         |  ↔/↑/↓  |    ✅    |    ✅    | 基于HTTP/3和QUIC协议，支持单向/双向流和消息传输，适应高丢包网络         |
| [`BroadcastChannel`](BroadcastChannel.md) |    ◎     |    ✅    |    ❌    | 同源标签页间广播通信（类似消息总线）                                    |
| [`SharedWorker`](SharedWorker.md)         |    ⇄     |    ✅    |    ❌    | 多标签页共享的 `WebWorker`，通过端口通信                                |
| [`MessageChannel`](MessageChannel.md)     |    ⇄     |    ✅    |    ❌    | 点对点通信管道，常用于主线程与 `WebWorker` 间通信                       |
