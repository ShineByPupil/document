...
- 宽高
  - `element.offsetWidth`/`element.offsetHeight` 元素的实际占用空间
  - `element.clientWidth`/`element.clientHeight` 元素内部可用空间
  - `element.scrollWidth`/`element.scrollHeight` 元素内容的完整大小
  - `element.getBoundingClientRect()` 元素的大小和位置
  - `window.getComputedStyle(element)` 元素的精确样式值
- 位置
  - `element.offsetTop`/`element.offsetLeft` 相对于父容器的位置
  - `element.scrollTop`/`element.scrollLeft` 内容的滚动位置
  - `element.getBoundingClientRect()` 相对于视口的位置(`scrollX`/`scrollY` 辅助使用)
